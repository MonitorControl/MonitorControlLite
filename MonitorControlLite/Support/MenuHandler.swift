...
class MenuHandler: NSMenu, NSMenuDelegate {
  ...
  func updateMenus(dontClose: Bool = false) {
    ...
    displays.append(contentsOf: DisplayManager.shared.getAllDisplays())
  ...
  func addDefaultMenuOptions() {
    if !DEBUG_MACOS10, #available(macOS 11.0, *), prefs.integer(forKey: PrefKey.menuItemStyle.rawValue) == MenuItemStyle.icon.rawValue {
      ...
      let viewWidth = max(86, self.size.width)
      ...
      preferencesIcon.frame = NSRect(x: menuItemView.frame.maxX - iconSize * 2 - 10 - 16 + compensateForBlock, y: menuItemView.frame.origin.y + 5, width: iconSize, height: iconSize)
